...
class TestChecklists:
    ...
    def another_user(self, django_user_model, default_icons):
        return django_user_model.objects.create(
            username="anothertest", email="another.test@test.com"
        )

    @pytest.fixture
    def tasks_abc(self, user):
        cl = Checklist.objects.create(
            id=1, name="test", created_by=user, order=1
        )
        Task.objects.create(
            checklist=cl, description="a", created_by=user, order=3
        )
        Task.objects.create(
            checklist=cl, description="b", created_by=user, order=1
        )
        Task.objects.create(
            checklist=cl, description="c", created_by=user, order=2
        )
    ...
    def test_checklist_should_return_only_users_checklist(
        self, client: Client, user, another_user
    ):
        ...
        assert response.json()["results"] == [
            {"id": checklist1.id, "name": checklist1.name}
        ]

    def test_task_should_return_only_users_checklist(
        self, client: Client, user, another_user
    ):
        checklist1 = Checklist.objects.create(name="test", created_by=user)
        task1 = Task.objects.create(
            checklist=checklist1, description="test", created_by=user
        )
        checklist2 = Checklist.objects.create(
            name="test2", created_by=another_user
        )
        Task.objects.create(
            checklist=checklist2, description="test", created_by=another_user
        )
        client.force_login(user)
    ...
    def test_task_check_should_succeed(self, client: Client, user):
        checklist: Checklist = Checklist.objects.create(
            name="test", created_by=user
        )
        task: Task = Task.objects.create(
            checklist=checklist, description="test", created_by=user
        )
        client.force_login(user)
    ...
    def test_change_checklist_order_should_succeed(self, client: Client, user):
        for i in range(1, 5):
            Checklist.objects.create(
                id=i, name="test", created_by=user, order=i
            )
        client.force_login(user)
        response = client.post(
    ...
    def test_change_task_order_should_succeed(self, client: Client, user):
        ...
        for i in range(1, 5):
            Task.objects.create(
                checklist=cl, description="test", created_by=user, order=i
            )
        client.force_login(user)
    ...
    def test_checklist_sort_by_description_asc_should_succeed(
        self, client: Client, user, tasks_abc
    ):
        ...
        assert response.status_code == 204
        assert ["a", "b", "c"] == list(
            checklist.task_set.all()
            .order_by("order")
            .values_list("description", flat=True)
        )

    def test_checklist_sort_by_description_desc_should_succeed(
        self, client: Client, user, tasks_abc
    ):
        checklist = Checklist.objects.filter(created_by=user).first()
        client.force_login(user)
    ...
    def test_checklist_sort_by_created_at_asc_should_succeed(
        self, client: Client, user, tasks_abc
    ):
        checklist = Checklist.objects.filter(created_by=user).first()
        client.force_login(user)
    ...
    def test_checklist_sort_by_created_at_desc_should_succeed(
        self, client: Client, user, tasks_abc
    ):
        checklist = Checklist.objects.filter(created_by=user).first()
        client.force_login(user)
    ...
    def test_checklist_sort_by_invalid_field_should_dail(
        self, client: Client, user, tasks_abc
    ):
        checklist = Checklist.objects.filter(created_by=user).first()
        client.force_login(user)
