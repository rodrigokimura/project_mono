...
class CardOrderViewTests(TestCase):
    def setUp(self):
        Icon.create_defaults()
        self.user = User.objects.create(
            username="test_card_order",
            email="test_card_order@test.com",
            password="top_secret",
        )
    ...
    def test_context_data_with_all_cards_in_config(self):
        user = User.objects.create(
            username="test_card_order_2",
            email="test_card_order_2@test.com",
            password="top_secret",
        )
    ...
    def test_context_data_with_two_cards_in_config(self):
        user = User.objects.create(
            username="test_card_order_3",
            email="test_card_order_3@test.com",
            password="top_secret",
        )
...
class TransactionListViewTests(TestCase):
    ...
    def test_transaction_month_archive_view_with_filters(self):
        ...
        month = timezone.now().strftime("%m")
        response = c.get(
            f"/fn/transactions/{year}/{month}/?category=1&account=1"
        )
        self.assertEqual(response.status_code, 200)
    ...
    def test_recurrenttransaction_delete_view(self):
        ...
        c.force_login(self.user)
        response = c.post(
            f"/fn/recurrent-transaction/{recurrent_transaction.id}/delete/"
        )
        self.assertEqual(response.status_code, 302)
        self.assertFalse(
            RecurrentTransaction.objects.filter(
                id=recurrent_transaction.id
            ).exists()
        )
    ...
    def test_installment_delete_view(self):
        ...
        self.assertEqual(response.status_code, 302)
        self.assertFalse(
            RecurrentTransaction.objects.filter(id=installment.id).exists()
        )
    ...
    def test_account_update_view(self):
        ...
        c.force_login(self.user)
        response = c.get(
            f"/fn/account/{Account.objects.filter(created_by=self.user).first().id}/"
        )
        self.assertEqual(response.status_code, 200)
...
class InviteTests(TestCase):
    ...
    def test_invite_api_existing_group_and_email(self):
        ...
        self.assertEqual(
            ...
            "You've already invited test3@test.com to this group.",
        )
