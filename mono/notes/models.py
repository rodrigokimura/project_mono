...
class Note(models.Model):
    ...
    title = models.CharField(
        max_length=100, verbose_name=_("title"), help_text="Note's title."
    )
    ...
    def save(self, *args, **kwargs):
        self.location = "/".join(
            map(lambda s: s.strip(), self.location.strip("/").split("/"))
        )
...
class Tag(models.Model):
    ...
    name = models.CharField(
        max_length=100, verbose_name=_("name"), help_text="Tag's name."
    )
    created_by = models.ForeignKey(
        User,
        on_delete=models.CASCADE,
