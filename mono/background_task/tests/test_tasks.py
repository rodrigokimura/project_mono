...
class TestBackgroundDecorator(TransactionTestCase):
    ...
    def test_default_name(self):
        proxy = tasks.background()(empty_task)
        self.assertEqual(
            proxy.name, "background_task.tests.test_tasks.empty_task"
        )

        proxy = tasks.background()(record_task)
        self.assertEqual(
            proxy.name, "background_task.tests.test_tasks.record_task"
        )
...
class TestTaskSchedule(TransactionTestCase):
    ...
    def test_create(self):
        ...
        self._within_one_second(
            schedule.run_at, timezone.now() + timedelta(seconds=10)
        )
    ...
    def test_merge(self):
        ...
        self._within_one_second(
            timezone.now() + timedelta(seconds=20), schedule.run_at
        )
        self.assertEqual(2, schedule.priority)
        self.assertEqual(TaskSchedule.RESCHEDULE_EXISTING, schedule.action)
        ...
        self._within_one_second(
            timezone.now() + timedelta(seconds=10), schedule.run_at
        )
        self.assertEqual(0, schedule.priority)
        self.assertEqual(TaskSchedule.RESCHEDULE_EXISTING, schedule.action)
...
class TestTaskModel(TransactionTestCase):
    ...
    def test_lock_expired(self):
        ...
        expire_by = timedelta(
            seconds=(app_settings.BACKGROUND_TASK_MAX_RUN_TIME + 2)
        )
...
class RepetitionTestCase(TransactionTestCase):
    ...
    def test_repeat(self):
        ...
        self.assertEqual(
            (new_task.run_at - old_task.run_at), timedelta(hours=1)
        )
...
class QuerySetManagerTestCase(TransactionTestCase):
    ...
    def test_completed_task_manager(self):
        ...
        self.assertEqual(
            len(CompletedTask.objects.created_by(self.user1).failed()), 0
        )
        self.assertEqual(
        ...
        self.assertEqual(
            len(CompletedTask.objects.created_by(self.user1).failed()), 1
        )
        self.assertEqual(
